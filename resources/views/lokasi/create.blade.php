...
<div class="container mx-auto px-4 py-8">
    <!-- Header -->
    <div class="mb-6">
        <a href="{{ route('admin.lokasi.index') }}" class="text-purple-600 hover:text-purple-800 flex items-center mb-4">
            <i class="fas fa-arrow-left mr-2"></i>
            Kembali
        </a>
        <h1 class="text-2xl font-bold text-gray-800">Tambah Lokasi Presensi</h1>
        <p class="text-gray-600">Tambahkan lokasi baru untuk presensi pegawai</p>
    </div>
        ...
        <form action="{{ route('admin.lokasi.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="number" name="radius" id="radius" value="{{ old('radius', 100) }}" min="1"
            ...
            <div class="mb-6">
                ...
                <p class="text-gray-500 text-sm mb-3">Klik pada peta untuk menandai lokasi presensi</p>
                <div id="map"></div>
            </div>

            <!-- Latitude & Longitude (Hidden/Readonly - auto-filled from map) -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label class="block text-gray-700 font-medium mb-2">
                ...
                <div>
                    ...
                    <input type="text" name="long" id="long" value="{{ old('long') }}" readonly
...
<script>
    // Initialize map centered on Banjarmasin with maximum zoom
    var map = L.map('map').setView([-3.3167, 114.5900], 13);
    ...
    // If lat/lng have values (e.g., from old input), show marker
    @if(old('lat') && old('long'))
        var savedLat = parseFloat('{{ old('lat') }}');
        var savedLng = parseFloat('{{ old('long') }}');
        if (!isNaN(savedLat) && !isNaN(savedLng)) {
            addMarker(savedLat, savedLng);
