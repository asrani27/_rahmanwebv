...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Initialize map centered on South Kalimantan
        map = L.map('map').setView([-2.5489, 114.7217], 9);
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="glass-effect rounded-2xl shadow-xl p-6 mb-6 border border-white/20">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 mb-2">Tambah Lokasi Presensi Baru</h1>
                <p class="text-gray-600">Tambahkan lokasi presensi baru untuk SKPD: {{ $skpd->nama }}</p>
            </div>
    ...
    <div class="glass-effect rounded-2xl shadow-xl p-6 border border-white/20">
        <form action="{{ route('skpd.lokasi.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="lat" id="lat" value="{{ old('lat') }}" required step="any"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all"
                           placeholder="Contoh: -3.319543" readonly>
                    <p class="mt-1 text-xs text-gray-500">Klik pada peta untuk mengisi koordinat</p>
                    @error('lat')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="long" id="long" value="{{ old('long') }}" required step="any"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all"
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="mt-2 text-xs text-gray-500">Klik pada peta untuk menentukan lokasi presensi. Lingkaran akan menunjukkan radius presensi.</p>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Radius (meter)</label>
                    <input type="number" name="radius" id="radius" value="{{ old('radius', 100) }}" required min="1"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all"
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="border border-gray-300 rounded-lg p-4 max-h-60 overflow-y-auto">
                        ...
                        @else
                            <p class="text-gray-500 text-sm">Belum ada pegawai di SKPD ini. Tambahkan pegawai terlebih dahulu.</p>
                        @endif
                    </div>
